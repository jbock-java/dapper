 ...
 * implementation is to be generated from a set of {@code #modules}. The generated class will
 ...
 * {@code Provider provision} or {@code MembersInjector members-injection} contracts.
 *
 * <p><a id="provision-methods"></a>
 *
 * <h3>Provision methods</h3>
 *
 * <p>Provision methods have no parameters and return an {@code Inject injected} or {@code Provides
 * provided} type. Each method may have a {@code Qualifier} annotation as well. The following are
 ...
 * </code></pre>
 *
 * <p>Provision methods, like typical {@code Inject injection} sites, may use {@code Provider} or
 * {@code Lazy} to more explicitly control provision requests. A {@code Provider} allows the user of
 * the component to request provision any number of times by calling {@code Provider#get}. A {@code
 * Lazy} will only ever request a single provision, but will defer it until the first call to {@code
 ...
 * {@code Inject}-annotated fields and methods of the passed instance. A members-injection method
 ...
 * </code></pre>
 *
 * <p>A method with no parameters that returns a {@code MembersInjector} is equivalent to a members
 * injection method. Calling {@code MembersInjector#injectMembers} on the returned object will
 ...
 *
 * <p>While a members-injection method for a type will accept instances of its subtypes, only {@code
 ...
 *
 * <p>If a nested {@code Builder @Component.Builder} or {@code Factory @Component.Factory} type
 * exists in the component, Dagger will generate an implementation of that type. If neither exists,
 * Dagger will generate a builder type that has a method to set each of the {@code #modules}
 * and component {@code #dependencies} named with the <a
 ...
 * <h2>Scope</h2>
 *
 * <p>Each Dagger component can be associated with a scope by annotating it with the {@code
 ...
 * responsibility to instantiate new component instances when appropriate. A {@code Singleton}
 ...
 * <h3>Subcomponents</h3>
 *
 * <p>The simplest way to relate two components is by declaring a {@code Subcomponent}. A
 ...
 *
 * <p>Subcomponents are declared by listing the class in the {@code Module#subcomponents()}
 * attribute of one of the parent component's modules. This binds the {@code Subcomponent.Builder}
 * or {@code Subcomponent.Factory} for that subcomponent within the parent component.
 *
 ...
 * interface</em> by declaring a {@code #dependencies component dependency}. When a type is
...
public @interface Component {
   ...
   * component implementation. Note that through the use of {@code Module#includes} the full set of
   ...
   *
   * <p>A builder is a type with setter methods for the {@code Component#modules modules},
   * {@code Component#dependencies dependencies} and {@code BindsInstance bound instances}
   ...
   *   <li>There <i>must</i> be a setter method for each {@code Component#dependencies
   *       component dependency}.
   *   <li>There <i>must</i> be a setter method for each non-{@code abstract} {@code
   ...
   *       bind the instance passed to them within the component. See {@code
   ...
   * is called. The parameters of that method allow the caller to provide the {@code
   * Component#modules modules}, {@code Component#dependencies dependencies} and {@code
   ...
   *   <li>The method <i>must</i> have a parameter for each {@code Component#dependencies
   *       component dependency}.
   *   <li>The method <i>must</i> have a parameter for each non-{@code abstract} {@code
   *       Component#modules module} that has non-{@code static} binding methods, unless Dagger can
   *       instantiate that module with a visible no-argument constructor.
   ...
   *       parameters bind the instance passed for that parameter within the component. See {@code
   *       BindsInstance @BindsInstance} for more information.
   *   <li>There <i>may</i> be non-{@code abstract} methods, but they are ignored as far as
